...
export async function sendCriticalAlert(req: Request, res: Response) {
  try {
    const { patientId, alertType, data, careTeam } = req.body;
    ...
    console.log(
      `🚨 Critical alert triggered: ${alertType} for patient ${patientId}`,
    );
...
export async function sendAppointmentReminders(req: Request, res: Response) {
  try {
    ...
    for (const appointment of appointments) {
      const template =
        reminderType === "24h" ? "appointment_24h" : "appointment_2h";
...
export async function sendMedicationReminder(req: Request, res: Response) {
  try {
    const { patientId, patientPhone, medicationName, dosage, dueTime } =
      req.body;
    ...
    if (!patientId || !patientPhone || !medicationName || !dosage) {
      return res.status(400).json({
        success: false,
        error:
          "PatientId, patientPhone, medicationName, and dosage are required",
      });
    }
    ...
    AuditLogger.logMedicationEvent(patientId, "reminder_sent", medicationName, {
      dosage,
      dueTime,
      success: result.success,
      messageId: result.messageId,
...
export async function sendDeviceAlert(req: Request, res: Response) {
  try {
    const { patientId, patientPhone, deviceName, alertType, data } = req.body;

    if (!patientId || !patientPhone || !deviceName || !alertType) {
      return res.status(400).json({
        success: false,
        error:
          "PatientId, patientPhone, deviceName, and alertType are required",
      });
    }
...
export async function sendCarePlanUpdate(req: Request, res: Response) {
  try {
    const { patientId, patientPhone, updateType, clinicPhone } = req.body;
...
export async function testMessagingService(req: Request, res: Response) {
  try {
    ...
    if (provider === "telnyx" || provider === "both") {
      const testMessage =
        "Test message from healthcare platform - Telnyx service check";
    ...
    if (provider === "twilio" || provider === "both") {
      const testMessage =
        "Test message from healthcare platform - Twilio service check";
      ...
      services: {
        telnyx: {
          ...
          phoneNumber: process.env.TELNYX_PHONE_NUMBER
            ? process.env.TELNYX_PHONE_NUMBER.replace(
                /(\d{1})(\d{3})(\d{3})(\d{4})/,
                "$1-$2-$3-****",
              )
            : "Not configured",
        ...
        twilio: {
          ...
          phoneNumber: process.env.TWILIO_PHONE_NUMBER
            ? process.env.TWILIO_PHONE_NUMBER.replace(
                /(\d{1})(\d{3})(\d{3})(\d{4})/,
                "$1-$2-$3-****",
              )
            : "Not configured",
