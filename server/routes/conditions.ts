...
import { validateCreateCondition, validateEntityId } from "../middleware/validation";
...
router.get("/", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  ...
  const data = patientId ? conditions.filter((c) => c.subject?.reference === `Patient/${patientId}`) : conditions;
  res.json({ success: true, data });
});

// Create condition
router.post("/", authenticateToken, validateCreateCondition, async (req: AuthenticatedRequest, res: Response) => {
  const { patientId, code, clinicalStatus, verificationStatus, onsetDateTime } = req.body || {};
...
router.put("/:id", authenticateToken, validateEntityId, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (idx === -1) return res.status(404).json({ success: false, error: "Condition not found" });
