...
import { validateCreateEncounter, validateEntityId } from "../middleware/validation";
...
router.get("/", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  ...
  const data = patientId ? encounters.filter((e) => e.subject?.reference === `Patient/${patientId}`) : encounters;
  res.json({ success: true, data });
});

router.post("/", authenticateToken, validateCreateEncounter, async (req: AuthenticatedRequest, res: Response) => {
  const { patientId, status = "in-progress", type, period, diagnosis, reasonCode } = req.body || {};
...
router.put("/:id", authenticateToken, validateEntityId, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (idx === -1) return res.status(404).json({ success: false, error: "Encounter not found" });
