...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!rule || !startDate) {
      return res
        .status(400)
        .json({ success: false, error: "rule and startDate are required" });
    }
    ...
    res.status(201).json({
      success: true,
      data: { seriesId, created: apptIds.length, appointmentIds: apptIds },
    });
  },
);
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!Array.isArray(appointmentIds) || appointmentIds.length === 0) {
      return res
        .status(400)
        .json({ success: false, error: "appointmentIds is required" });
    }
    res.json({
      success: true,
      data: {
