  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (existingMedications.rows.length > 0) {
        ...
        const existingDrugs = existingMedications.rows.map((m) =>
          m.name.toLowerCase(),
        );
        ...
        if (existingDrugs.includes("warfarin") && newDrug.includes("aspirin")) {
          interactionWarnings.push(
            "Potential interaction: Warfarin and Aspirin may increase bleeding risk",
          );
        }
        if (
          existingDrugs.includes("simvastatin") &&
          newDrug.includes("amiodarone")
        ) {
          interactionWarnings.push(
            "Potential interaction: Simvastatin and Amiodarone may increase muscle damage risk",
          );
        }
      }
    ...
    try {
      ...
      if (drugNames.includes("warfarin") && drugNames.includes("aspirin")) {
        interactions.push({
          ...
          recommendation:
            "Monitor closely and consider alternative medications",
        });
        riskLevel = "high";
      }

      if (
        drugNames.includes("simvastatin") &&
        drugNames.includes("amiodarone")
      ) {
        interactions.push({
    ...
    `;
      ...
      res.json({
        stats: {
          ...
          medicationsWithSideEffects: parseInt(
            medicationStats.medications_with_side_effects,
          ),
          medicationsWithInteractions: parseInt(
            medicationStats.medications_with_interactions,
          ),
        },
      });
    } catch (error) {
