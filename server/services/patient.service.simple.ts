...
export class SimplePatientService {
      ...
      for (const patientData of samplePatients) {
        try {
          ...
          if (existingUser.length === 0) {
            ...
            console.log(
              `Created sample patient: ${patientData.firstName} ${patientData.lastName}`,
            );
          }
        } catch (error: any) {
  ...
  static getMockIntakePatient(patientId: string): any {
    ...
    console.log(
      `[MockPatients] Mock patient lookup result for ${patientId}:`,
      result ? "Found" : "Not found",
    );
    if (result) {
      console.log(
        `[MockPatients] Found patient: ${result.firstName} ${result.lastName}`,
      );
    }
    return result;
  }
   ...
   */
  static getMockPatientsForSearch(
    filters: any = {},
    page: number = 1,
    limit: number = 20,
  ): any {
