...
export class CareTeamService {
  ...
  async sendAlert(
    ...
    console.log(
      `🚨 Sending ${context.alertType} alert for patient ${careTeam.patientName}`,
    );
  ...
  private scheduleEscalation(
    ...
    const level1Timer = setTimeout(
      async () => {
        console.log(
          `⚠️ Level 1 escalation for patient ${careTeam.patientName}`,
        );
    ...
    const level2Timer = setTimeout(
      async () => {
        console.log(
          `🚨 Level 2 escalation for patient ${careTeam.patientName}`,
        );
