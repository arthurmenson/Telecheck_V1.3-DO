...
export class TwilioService {
  ...
  constructor() {
    ...
    if (!this.accountSid || !this.authToken) {
      console.warn(
        "⚠️ Twilio credentials not configured. Backup SMS and call services will be disabled.",
      );
    }
  }
    ...
    try {
      ...
      const smsData: TwilioSMSRequest = {
        ...
        StatusCallback:
          ...
          `${process.env.BASE_URL || "http://localhost:3000"}/api/webhooks/twilio/sms`,
      };

      console.log(
        `📱 Sending SMS via Twilio (backup) to ${formattedTo}:`,
        message.substring(0, 50) + "...",
      );

      const response = await this.makeRequest<TwilioSMSResponse>(
        "/Messages.json",
        "POST",
        smsData,
      );
  ...
  async makeCall(
    ...
    try {
      ...
      const callData: TwilioCallRequest = {
        ...
        StatusCallback:
          options?.statusCallback ||
          `${process.env.BASE_URL || "http://localhost:3000"}/api/webhooks/twilio/call`,
      ...
      const response = await this.makeRequest<TwilioCallResponse>(
        "/Calls.json",
        "POST",
        callData,
      );
  ...
  async sendVoiceMessage(
    ...
    try {
      ...
      if (callResult.success) {
        console.log(
          `🔊 Voice message queued via Twilio for call ${callResult.callId}:`,
          message.substring(0, 50) + "...",
        );
      }
