...
import { useCreatePrescription, useVerifyEpcs, useMedicationHistory, useSearchMedications, useCancelPrescription, useRefillPrescription, usePrescription, useOptimisticUpdate, queryKeys } from "@/hooks/api";
...
export function ErxComposer() {
  ...
  const [selectedMedication, setSelectedMedication] = useState<{ code?: string; display?: string } | null>(null);
    ...
    <div className="container mx-auto p-4 space-y-4">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input id="patientId" value={patientId} onChange={(e) => setPatientId(e.target.value)} placeholder="Patient/123" />
            </div>
            <div>
            ...
            <div className="md:col-span-2 space-y-2">
              ...
              {searchQuery.length > 2 && (
                <div className="border rounded p-2 max-h-48 overflow-auto bg-background">
                  {search.isLoading && <div className="text-xs text-muted-foreground">Searching…</div>}
                  {!search.isLoading && (search.data?.length ? (
                      ...
                      {search.data.map((m: any) => (
                        <li key={m.id}>
                          <button
                            ...
                            onClick={() => {
                              const display = m.name || m.text || m.generic || String(m.id);
                              const code = m.rxnormCode || m.id || m.code;
                              setMedicationText(display);
                              setSelectedMedication({ code: String(code), display });
                            }}
                          >
                            <div className="text-sm font-medium">{m.name || m.text || m.id}</div>
                            {m.generic && <div className="text-xs text-muted-foreground">{m.generic}</div>}
                          </button>
                        </li>
                      ))}
                    </ul>
            ...
            <div className="md:col-span-2">
              ...
              <Textarea id="dosage" value={dosage} onChange={(e) => setDosage(e.target.value)} placeholder="1 capsule PO TID for 7 days" />
            </div>
          </div>
          <div className="flex gap-2">
            <Button onClick={handleCreate} disabled={createRx.isPending}>Create Prescription</Button>
            <Button variant="secondary" onClick={handleVerify} disabled={verify.isPending}>Verify EPCS</Button>
          </div>
          {createRx.error && <div className="text-red-600 text-sm">Failed: {(createRx.error as any)?.message || "Error"}</div>}
          {verify.error && <div className="text-red-600 text-sm">EPCS Failed: {(verify.error as any)?.message || "Error"}</div>}
          {createRx.data?.data && (
            <pre className="bg-muted p-3 rounded text-xs overflow-auto">{JSON.stringify(createRx.data.data, null, 2)}</pre>
          )}
          {currentRxId && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {(rx as any)?.data?.status && (
                <Badge variant={(rx as any)?.data?.status === 'stopped' ? 'destructive' : 'secondary'}>
                  {(rx as any)?.data?.status}
                </Badge>
              )}
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {!patientId && <div className="text-sm text-muted-foreground">Enter a Patient ID to load history.</div>}
          {patientId && (
            <>
                ...
                {(history as any)?.map?.((h: any) => (
                  <div key={h.id} className="flex items-center justify-between border rounded p-2">
                    <div className="space-y-0.5">
                      <div className="text-sm font-medium">{h.medicationCodeableConcept?.text || h.id}</div>
                      <div className="text-xs text-muted-foreground">{new Date(h.authoredOn || Date.now()).toLocaleString()}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant={h.status === 'completed' ? 'secondary' : 'outline'}>{h.status || 'unknown'}</Badge>
                      <Button size="sm" variant="secondary" onClick={() => setCurrentRxId(h.id)}>Select</Button>
                    </div>
                  </div>
                ))}
                {!(history as any)?.length && (
                  <div className="text-xs text-muted-foreground">No history entries.</div>
                )}
              </div>
              <div>
                <div className="text-xs text-muted-foreground mb-1">Raw</div>
                <pre className="bg-muted p-3 rounded text-xs overflow-auto">{JSON.stringify((history as any) ?? [], null, 2)}</pre>
              </div>
            </>
          )}
        </CardContent>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <div className="md:col-span-2">
              ...
              <Input id="rxid" value={currentRxId} onChange={(e) => setCurrentRxId(e.target.value)} placeholder="rx_..." />
            </div>
            <div className="flex gap-2">
              <Button
                type="button"
                variant="secondary"
                onClick={() => currentRxId && refetchQueries(queryKeys.erx.prescription(currentRxId))}
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  if (patientId) {
                    updateCache<any[]>(queryKeys.erx.history(patientId), (old = []) =>
                      old.map((item: any) => item.id === currentRxId ? { ...item, status: 'stopped' } : item)
                    );
                  }
                  updateCache<any>(queryKeys.erx.prescription(currentRxId), (old: any = {}) => ({ ...old, status: 'stopped' }));
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  if (patientId) {
                    updateCache<any[]>(queryKeys.erx.history(patientId), (old = []) =>
                      old.map((item: any) => item.id === currentRxId ? { ...item, status: item.status || 'active', refillRequested: true } : item)
                    );
                  }
                  updateCache<any>(queryKeys.erx.prescription(currentRxId), (old: any = {}) => ({ ...old, refillRequested: true }));
          ...
          {currentRxId && (
            <pre className="bg-muted p-3 rounded text-xs overflow-auto">{JSON.stringify((rx as any)?.data ?? {}, null, 2)}</pre>
          )}
