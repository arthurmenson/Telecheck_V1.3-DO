...
class MessagingAdminService {
  ...
  async getAnalytics(period: "24h" | "7d" | "30d" = "24h"): Promise<{
    success: boolean;
  ...
  async getCareTeamConfig(): Promise<{
    success: boolean;
    careTeam?: { members: CareTeamMember[]; escalationRules: any[] };
    error?: string;
  }> {
    try {
  ...
  async getAuditLogs(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/audit-logs?${searchParams}`,
      );
      const data = await response.json();
      return data;
    } catch (error) {
