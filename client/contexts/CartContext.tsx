...
type CartAction =
  ...
  | {
      ...
      payload: { id: string; method: CartItem["shippingMethod"] };
    }
  | {
      type: "TOGGLE_SUBSCRIPTION";
      payload: { id: string; frequency?: CartItem["subscriptionFrequency"] };
    }
  ...
  switch (action.type) {
    case "ADD_ITEM": {
      ...
      if (existingItemIndex >= 0) {
        ...
        newItems = [
          ...state.items,
          { ...action.payload, id: `cart_${Date.now()}` },
        ];
      }

      const summary = CartCalculator.calculateCartSummary(
        newItems,
        state.promotions,
      );
    ...
    case "REMOVE_PROMOTION": {
      const newAppliedPromotions = state.appliedPromotions.filter(
        (id) => id !== action.payload,
      );
      const summary = CartCalculator.calculateCartSummary(
        state.items,
...
const CartContext = createContext<
  | {
      ...
      setShippingMethod: (
        id: string,
        method: CartItem["shippingMethod"],
      ) => void;
      toggleSubscription: (
        id: string,
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = (item: Omit<CartItem, "id">) => {
    dispatch({
      type: "ADD_ITEM",
      payload: { ...item, id: `cart_${Date.now()}` },
    });
  };
