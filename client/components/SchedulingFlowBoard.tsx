        ...
        <div className="flex gap-4">
          {statuses.map((status) => (
            <div key={status} className="flex-1">
              <div className="font-semibold mb-2 capitalize">{status.replace("_", " ")}</div>
              <div className="space-y-2">
                {items.filter((i) => i.status === status).map((i) => (
                  <div key={i.id} className="p-2 rounded border text-sm">
                    <div>{i.time} • {i.patient}</div>
                    <div className="text-muted-foreground">{i.provider} • Room {i.room}</div>
                  </div>
                ))}
              </div>
            </div>
