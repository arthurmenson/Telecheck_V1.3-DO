...
export function AIProductSearch({
  ...
  useEffect(() => {
    if (searchQuery.length > 2) {
      const timer = setTimeout(() => {
        ...
        const aiSuggestions = SAMPLE_SUGGESTIONS.filter(
          (suggestion) =>
  ...
  const getRelevanceReasons = (product: Product, query: string): string[] => {
    ...
    if (
      product.indications.some((ind) => ind.toLowerCase().includes(queryLower))
    ) {
      ...
      <CommandDialog open={isCommandOpen} onOpenChange={setIsCommandOpen}>
        ...
        <CommandList className="max-h-[400px]">
          <CommandEmpty>
            <div className="text-center py-6">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Try searching for conditions, symptoms, or product names
              </p>
            </div>
          </CommandEmpty>
          ...
          {aiSearchResults.length > 0 && (
            <CommandGroup
              heading={`Search Results (${aiSearchResults.length})`}
            >
