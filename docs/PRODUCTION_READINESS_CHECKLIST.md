...
- [~] **Secrets Management**: Store credentials and keys in a managed vault service with rotation policies. — _Progress: Messaging configuration now persists secret manager references instead of raw credentials; Next: select a vault provider and wire `server/config/database.ts` plus provider SDK clients to resolve the references at runtime._
