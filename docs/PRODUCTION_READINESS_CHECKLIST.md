...
- [~] **Performance & Load Testing**: Benchmark API and realtime services against target concurrency and data volume. — _Progress: Added a k6 patient API load script (`npm run test:load`) with defined thresholds and documented the [Performance & Load Test Plan](./PERFORMANCE_TEST_PLAN.md); Next: expand coverage to telehealth/messaging scenarios and automate staged runs with dashboard exports._
