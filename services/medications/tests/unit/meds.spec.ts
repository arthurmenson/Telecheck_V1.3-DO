...
describe('Medications Service', () => {
  ...
  it('GET /medications/interactions returns expected interaction', async () => {
    const res = await server.inject({ method: 'GET', url: '/medications/interactions?drugA=lipitor&drugB=warfarin' });
    expect(res.statusCode).toBe(200);
    const body = res.json() as any;
  ...
  it('GET /medications/search returns lipitor when q matches', async () => {
    const res = await server.inject({ method: 'GET', url: '/medications/search?q=lipitor' });
    expect(res.statusCode).toBe(200);
    const body = res.json() as any;
