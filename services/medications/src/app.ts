...
server.get('/medications/interactions', async (request, reply) => {
  ...
  if ((a === 'lipitor' && b === 'warfarin') || (a === 'warfarin' && b === 'lipitor')) {
    reply.send({ interactions: [{ pair: [a, b], severity: 'major', note: 'Monitor INR closely' }] });
    return;
  }
...
server.get('/medications/search', async (request, reply) => {
  ...
  if (q.includes('lipitor')) {
    reply.send({ items: [{ id: 'lipitor', name: 'Lipitor', generic: 'atorvastatin' }], q });
    return;
  }
  reply.send({ items: [], q });
});
...
// Health
server.get('/health', async () => ({ status: 'healthy', service: 'medications', timestamp: new Date().toISOString() }));

async function start() {
  await registerPlugins();
  await server.listen({ port: config.port, host: config.host });
  server.log.info(`💊 Medications service running on http://${config.host}:${config.port}`);
}
