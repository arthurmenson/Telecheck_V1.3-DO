...
const config = {
  ...
  services: {
    ...
    messagingAdmin:
      process.env.MESSAGING_ADMIN_SERVICE_URL || "http://localhost:3010",
...
const skipAuth =
  process.env.SKIP_AUTH === "true" || config.nodeEnv !== "production";
...
async function start() {
  try {
    ...
    server.log.info(
      `🚀 API Gateway running on http://${config.host}:${config.port}`,
    );
    server.log.info(
