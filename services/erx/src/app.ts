...
server.post('/erx/prescriptions', async (request, reply) => {
  ...
  if (!body.patientId || !body.medication) {
    reply.status(400).send({ success: false, error: 'patientId and medication are required' });
    return;
  }
  ...
  reply.status(201).send({
    ...
    data: {
      ...
      requester: { reference: `Practitioner/${body.requestedByUserId || 'current'}` },
...
// Health
server.get('/health', async () => ({ status: 'healthy', service: 'erx', timestamp: new Date().toISOString() }));

async function start() {
  await registerPlugins();
  await server.listen({ port: config.port, host: config.host });
  server.log.info(`📝 eRx service running on http://${config.host}:${config.port}`);
}
