...
export async function thresholdsRoutes(
  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        const { patientId, vital, enabled } = query;
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (
          ...
          threshold.patientId !== request.user.id
        ) {
          reply.status(403).send({
            success: false,
            error: "Access denied",
  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        const { age, conditions, gender } = query;
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      try {
        ...
        reply.send({
          ...
          status:
            violations.length > 0 ? "violations_detected" : "within_thresholds",
        });
      } catch (error) {
...
async function getThresholds(params: {
  ...
  if (params.vital) {
    filteredThresholds = filteredThresholds.filter(
      (t) => t.vital === params.vital,
    );
  }

  if (params.enabled !== undefined) {
    filteredThresholds = filteredThresholds.filter(
      (t) => t.enabled === params.enabled,
    );
  }
