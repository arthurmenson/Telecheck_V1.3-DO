...
export async function vitalsRoutes(
  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        const { patientId, timeframe = "7d", vitals } = query;
  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        const { patientId, limit = 20, offset = 0 } = query;
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      try {
        ...
        const newVitals = await createVitalSigns({
          ...
          recordedAt: vitalsData.recordedAt
            ? new Date(vitalsData.recordedAt)
            : new Date(),
        });
  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        const { patientId, period = "30d" } = query;
...
async function checkVitalsThresholds(vitals: any): Promise<void> {
  ...
  if (vitals.heartRate && (vitals.heartRate < 60 || vitals.heartRate > 100)) {
    alerts.push({
      type: "vitals",
      severity:
        vitals.heartRate < 40 || vitals.heartRate > 140 ? "critical" : "medium",
