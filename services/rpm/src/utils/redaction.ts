...
export function redactVitalsData(vitals: any): any {
  ...
  return {
    id: vitals.id,
    patientId: vitals.patientId ? `[PATIENT_${vitals.patientId.slice(-4)}]` : '[PATIENT_UNKNOWN]',
    recordedAt: vitals.recordedAt || '[UNKNOWN_TIME]',
    source: vitals.source || 'unknown',
    vitalTypes: Object.keys(vitals).filter(key => 
      ['heartRate', 'bloodPressureSystolic', 'bloodPressureDiastolic', 
       'temperature', 'oxygenSaturation', 'weight', 'height'].includes(key) &&
      vitals[key] !== undefined && vitals[key] !== null
    ),
    hasVitals: true
  };
}
...
export function redactAlertData(alert: any): any {
  ...
  return {
    id: alert.id,
    patientId: alert.patientId ? `[PATIENT_${alert.patientId.slice(-4)}]` : '[PATIENT_UNKNOWN]',
    type: alert.type || 'unknown',
    severity: alert.severity || 'unknown',
    title: alert.title ? `[ALERT_TITLE_${alert.title.length}_CHARS]` : '[NO_TITLE]',
...
export function redactThresholdData(threshold: any): any {
  ...
  return {
    id: threshold.id,
    patientId: threshold.patientId ? `[PATIENT_${threshold.patientId.slice(-4)}]` : '[PATIENT_UNKNOWN]',
...
export function redactDeviceData(device: any): any {
  ...
  return {
    id: device.id,
    patientId: device.patientId ? `[PATIENT_${device.patientId.slice(-4)}]` : '[PATIENT_UNKNOWN]',
...
export function redactRPMSummary(summary: any): any {
  ...
  return {
    patientId: summary.patientId ? `[PATIENT_${summary.patientId.slice(-4)}]` : '[PATIENT_UNKNOWN]',
