...
function signToken(user: any): string {
  ...
  return Buffer.from(JSON.stringify({ sub: user.id, email: user.email, role: user.role, permissions: [] })).toString('base64');
}
...
server.post('/auth/login', async (request, reply) => {
  ...
  reply.send({ user, token: signToken(user), refreshToken: 'rt_' + Date.now(), expiresIn: 3600 });
});
...
server.get('/health', async () => ({ status: 'healthy', service: 'auth', timestamp: new Date().toISOString() }));

async function start() {
  await registerPlugins();
  await server.listen({ port: config.port, host: config.host });
  server.log.info(`🔐 Auth service running on http://${config.host}:${config.port}`);
}
