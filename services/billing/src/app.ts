...
function buildX12_270(req: any): string {
  ...
  return [
    'ISA*00*          *00*          *ZZ*SENDER         *ZZ*RECEIVER       *' + now + '*1234*^*00501*000000905*0*T*:~',
    ...
    'HL*3*2*22*0~',
    'NM1*IL*1*' + (req?.subscriber?.lastName || 'DOE') + '*' + (req?.subscriber?.firstName || 'JOHN') + '****MI*' + (req?.subscriber?.memberId || 'W000000000') + '~',
    'DMG*D8*' + (req?.subscriber?.dob?.replace(/-/g, '') || '19700101') + '*' + (req?.subscriber?.gender || 'M') + '~',
    'DTP*291*D8*' + now + '~',
    'SE*10*0001~',
    'GE*1*1~',
    'IEA*1*000000905~'
...
function buildX12_837(claim: any): string {
  const now = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  return [
    'ISA*00*          *00*          *ZZ*SUBMITTER      *ZZ*PAYER          *' + now + '*1234*^*00501*000000906*0*T*:~',
...
server.get('/eligibility/271/:id', async (request, reply) => {
  ...
  // Stub parsed 271
  reply.send({ id, status: 'active', coverage: { plan: 'PPO', effective: '2024-01-01', copay: 20 } });
});
...
server.post('/billing/ack/999', async (request, reply) => {
  ...
  reply.send({ status: 'accepted', errors: [], controlNumber: ack?.controlNumber || '0001' });
});
...
server.post('/billing/835', async (request, reply) => {
  ...
  // Stub parse
  reply.send({ id: '835_' + Date.now(), payments: remit?.payments?.length || 0, posted: true });
});

server.get('/health', async () => ({ status: 'healthy', service: 'billing', timestamp: new Date().toISOString() }));

async function start() {
  await registerPlugins();
  await server.listen({ port: config.port, host: config.host });
  server.log.info(`💼 Billing/EDI service running on http://${config.host}:${config.port}`);
}
