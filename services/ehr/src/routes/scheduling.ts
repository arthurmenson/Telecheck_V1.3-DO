...
export async function schedulingRoutes(
   ...
   */
  fastify.get('/slots', async (request: AuthenticatedRequest, reply: FastifyReply) => {
   ...
   */
  fastify.post('/book', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    const query = request.query as any;
   ...
   */
  fastify.post('/:id/cancel', async (request: AuthenticatedRequest, reply: FastifyReply) => {
   ...
   */
  fastify.post('/:id/reschedule', async (request: AuthenticatedRequest, reply: FastifyReply) => {
    const { id } = request.params as { id: string };
    const query = request.query as any;
   ...
   */
  fastify.get('/error', async (request: FastifyRequest, reply: FastifyReply) => {
    reply.status(500).send({
      success: false,
...
async function getAvailableSlots(params: {
  ...
  if (params.date) {
    ...
    return baseSlots.filter(slot => 
      new Date(slot.start).toDateString() === targetDate
    );
  }
