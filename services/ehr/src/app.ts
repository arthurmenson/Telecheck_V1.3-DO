...
const config = {
  ...
  databaseUrl:
    ...
    "postgresql://postgres:password@localhost:5432/telecheck_ehr",
...
if (PrismaClientRef && useDb) {
  try {
    ...
    server.log.warn(
      { error },
      "Failed to connect to database, using mock client",
    );
...
export const BookingRequestSchema = z.object({
  ...
  patientId: z.string().uuid(),
  appointmentType: z
    .enum(["consultation", "follow-up", "urgent", "annual"])
    .optional(),
...
server.addHook("onResponse", async (request: AuthenticatedRequest, reply) => {
  const isPhiEndpoint = ["/patients/", "/intake", "/messaging/"].some(
...
async function start() {
  try {
    ...
    server.log.info(
      `🏥 EHR service running on http://${config.host}:${config.port}`,
    );
    server.log.info(
