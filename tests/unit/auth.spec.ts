...
describe('Auth routes', () => {
  ...
  it('registers a new user and returns tokens', async () => {
    ...
    ;(dbPool.query as any)
      ...
      .mockResolvedValueOnce({ rows: [{ id: 'u1', email, first_name: 'New', last_name: 'User', role: 'patient' }] }) // insert

    const res = await request(app)
      .post('/api/auth/register')
      .send({ email, password: 'Passw0rd!', firstName: 'New', lastName: 'User', role: 'patient' })
  ...
  it('logs in, then refreshes token', async () => {
    ...
    ;(dbPool.query as any)
      .mockResolvedValueOnce({ rows: [{ id: 'u2', email, password_hash: hash, first_name: 'A', last_name: 'B', role: 'patient', is_active: true }] }) // login select
    ...
    ;(dbPool.query as any)
      .mockResolvedValueOnce({ rows: [{ id: 'u2', email, first_name: 'A', last_name: 'B', role: 'patient', is_active: true }] }) // user by id
