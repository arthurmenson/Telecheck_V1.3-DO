...
// FHIR-aligned minimal clinical types (stubs)
...
export interface Encounter {
  ...
  diagnosis?: Array<{
    ...
    use?: CodeableConcept; // billing diagnosis
  }>;
...
export interface Condition {
  id: string;
  subject: Reference; // Patient
  code: CodeableConcept; // ICD-10-CM/SNOMED
...
export interface Immunization {
  id: string;
  patient: Reference;
  status: string; // completed | not-done
...
export interface MedicationRequest {
  id: string;
  status: string; // active | completed | stopped | draft
  intent: string; // order
