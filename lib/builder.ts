...
export async function ensureBuilder() {
  ...
  const KEY =
    (import.meta as any).env?.VITE_BUILDER_API_KEY ||
    (globalThis as any).NEXT_PUBLIC_BUILDER_API_KEY;
  if (!KEY)
    builderWarning(
      "[builder] Missing API key env: VITE_BUILDER_API_KEY / NEXT_PUBLIC_BUILDER_API_KEY",
    );
